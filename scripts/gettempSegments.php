	...
	$result = pg_query($dbconn, 
		...
		FROM ( SELECT 'FeatureCollection' AS type, 
			...
			FROM (	
				...
				ST_AsGeoJson(ST_Transform(k.line,4326))::json AS geometry, 

				row_to_json((SELECT seg_id FROM (SELECT id, sid, starttime) AS seg_id)) AS properties

				FROM (SELECT (LS_SplitUpTimeSegments(segment_linezm,starttime)::subsegment_holder).linezm as line, s.id as id, s.segment_id as sid, 
							(LS_SplitUpTimeSegments(segment_linezm,starttime)::subsegment_holder).mean_line_time as starttime FROM test_update_segment s) k
